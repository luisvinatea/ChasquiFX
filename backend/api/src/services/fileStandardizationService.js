 ...
 */
async function standardizeFilename(originalFile, metadataKeys, template, delimiter = '_') {
  try {
    // Extract metadata
    const metadata = await extractJsonMetadata(originalFile, metadataKeys);
    
    if (!metadata || Object.keys(metadata).length === 0 || 
        metadataKeys.some(k => metadata[k] === null || metadata[k] === undefined)) {
      logger.warning(`Could not extract required metadata from ${typeof originalFile === 'string' ? originalFile : 'object'}`);
      return null;
    }
...
async function writeStandardizedFile(data, type, baseDir = null) {
  try {
    ...
    if (type === 'flight') {
      ...
      targetDir = baseDir || path.resolve(__dirname, '../../../assets/data/flights');
    } else if (type === 'forex') {
      filename = await standardizeForexFilename(data);
      targetDir = baseDir || path.resolve(__dirname, '../../../assets/data/forex');
    } else {
