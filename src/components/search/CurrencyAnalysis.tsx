...
export function CurrencyAnalysis({ originCurrency, destinationCurrency }) {
  ...
  const fetchAnalysis = async () => {
    setLoading(true);
    try {
      const response = await fetch(`/api/currencies/${originCurrency}/${destinationCurrency}`);
  ...
  if (!originCurrency || !destinationCurrency) {
    return (
      <Card>
        ...
        <CardContent>
          <p className="text-muted-foreground">Select currencies to see forex analysis</p>
        </CardContent>
      </Card>
    );
  }
  ...
  const getAdvantageIcon = (advantage) => {
    if (advantage > 5) return <TrendingUp className="h-4 w-4 text-green-500" />;
    if (advantage < -5) return <TrendingDown className="h-4 w-4 text-red-500" />;
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="flex items-center justify-between">
          <span className="font-medium">{originCurrency} → {destinationCurrency}</span>
        ...
        <div className="space-y-2">
          <div className="flex justify-between text-sm">
            ...
            <span>{analysis.origin.overvaluationPercentage.toFixed(1)}% overvalued</span>
          </div>
          <div className="flex justify-between text-sm">
            <span>{analysis.destination.name}</span>
            <span>{analysis.destination.overvaluationPercentage.toFixed(1)}% overvalued</span>
          </div>
        </div>
